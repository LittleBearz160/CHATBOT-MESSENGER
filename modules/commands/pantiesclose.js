...
"https://konachan.net/image/efe6aa7bbeac98d05a98b31465b5fe65/Konachan.com%20-%20886%20ass%20close%20panties%20red%20skirt%20underwear%20upskirt.jpg?0.67499538349191",
  ...
  var money = data.money
      if(money < 1500) api.sendMessage("Bạn cần 1500$ để xem ảnh ?",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 1500})
   var callback = () => api.sendMessage({body:`Panties close😼\nHiện có: ${link.length} ảnh\nSố dư: -1500$`,attachment: fs.createReadStream(__dirname + "/cache/5.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/5.jpg")); 
