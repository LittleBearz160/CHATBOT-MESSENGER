...
var mention = Object.keys(event.mentions);
		...
		api.sendMessage(`✅ Đã xoá toàn bộ số exp của ${event.mentions[mention].replace("@", "")}\nSố exp xoá là ${expdel}.`, event.threadID, async() => {await Currencies.setData(mention, {exp: parseInt(kong)})});
			...
			return api.sendMessage({
				body: (`✅ Đã cộng thêm ${expSet} exp cho ${event.mentions[mention].replace("@", "")}`),
				mentions: [{
		...
		let nameeee = (await Users.getData(id)).name
		   return api.sendMessage(`✅ Đã cộng thêm ${cut} exp cho ${nameeee}`, event.threadID, async() => {await Currencies.setData(id, {exp: parseInt(cut)})}, event.messageID);
