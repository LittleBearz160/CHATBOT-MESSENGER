...
var mention = Object.keys(event.mentions)[0];
    ...
    var {body} = event;
			...
			var moneySet = content.substring(content.lastIndexOf(" ") + 1);
    			if (args[0]=='me'){
    			 return api.sendMessage(`✅ Đã cộng thêm ${moneySet} vào thẻ của bạn`, event.threadID, () => Currencies.increaseMoney(event.senderID, parseInt(moneySet)), event.messageID)	
			}
			else if(args[0]=="del"){
if (args[1] == 'me'){
...
var mention = Object.keys(event.mentions)[0];
		...
		api.sendMessage(`✅ Đã xoá toàn bộ số tiền của ${event.mentions[mention].replace("@", "")}\n💸 Số tiền xoá là ${moneydel}.`, event.threadID, async () => await Currencies.decreaseMoney(mention, parseInt(moneydel)));
			...
			return api.sendMessage({
				body: (`✅ Đã cộng thêm ${moneySet}$ vào thẻ của ${event.mentions[mention].replace("@", "")}`),
				mentions: [{
		...
		let nameeee = (await Users.getData(id)).name
		   return api.sendMessage(`✅ Đã cộng thêm ${cut}$ vào thẻ của`, event.threadID, () => Currencies.increaseMoney(id, parseInt(cut)), event.messageID)	
