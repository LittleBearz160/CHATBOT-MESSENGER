...
var min = Math.floor(Math.random() * 1);
  ...
  var money = data.money
      if(money < 1500) api.sendMessage("Cần 1500$ để xem ảnh",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 1500})
   var callback = () => api.sendMessage({body:`Loli cho các con vợ đây\nSố dư: -1500$ `,attachment: fs.createReadStream(__dirname + "/cache/1.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.jpg"), event.messageID); 
